...
const INITIAL: BoardPlayer[] = [
  ...
  { id: 'mendoza', tier: 1, name: 'Fernando Mendoza', pos: 'QB', team: 'LV', college: 'Indiana', pick: 1, s: ['PASS — 2023 (Cal): 1,708 yds, 14 TD, 10 INT, 63.0% | 2024 (Cal): 3,004 yds, 16 TD, 6 INT, 68.7% | 2025 (IND): 3,535 yds, 41 TD, 6 INT, 72.0%', 'RUSH — 2023: 49-86-2TD | 2024: 87-105-2TD | 2025: 90-276-7TD', 'MISC — 2025 Heisman winner; led Indiana to the national title; school-record 41 passing TD'] },
  ...
  { id: 'tyson', tier: 1, name: 'Jordyn Tyson', pos: 'WR', team: 'NO', college: 'Arizona State', pick: 8, s: ['REC  — 2022 (Colorado): 22-470-5TD (21.4) | 2023 (ASU): 3 G, season-ending knee injury | 2024 (ASU): 75-1101-10TD (14.7) | 2025 (ASU): 61-711-8TD (11.7)', 'RUSH — 2024: 5-42-0TD | 2025: 2-4-1TD', 'PASS — 2025: 0-for-1', 'MISC — Led Big 12 in receptions 2024-25 (136); 1,812 rec yds over two ASU seasons'] },
  ...
  { id: 'stowers', tier: 3, name: 'Eli Stowers', pos: 'TE', team: 'PHI', college: 'Vanderbilt', pick: 54, s: ['REC  — 2024: 49-638-5TD (13.0) | 2025: 62-769-4TD (12.4)', 'RUSH — 2024: 6-7-0TD | 2025: 2-2-0TD', 'MISC — Texas A&M → New Mexico State → Vanderbilt, Mackey Award profile, 1st-team AA'] },
  ...
  { id: 'bell-chris', tier: 5, name: 'Chris Bell', pos: 'WR', team: 'MIA', college: 'Louisville', pick: 94, s: ['REC  — 2024: 43-737-4TD (17.1) | 2025: 72-917-6TD (12.7)', 'MISC — Louisville outside WR; first-team All-ACC 2025; injury ended his season after 11 games'] },
  ...
  { id: 'young', tier: 5, name: 'Colbie Young', pos: 'WR', team: 'CIN', college: 'Georgia', pick: 140, s: ['REC  — 2023 (Miami): 47-563-5TD | 2024 (Georgia): 11-149-2TD | 2025 (Georgia): 26-358-1TD', 'MISC — Miami → Georgia transfer; leg injury ended 2025 early. Career 116-1437-13TD including 2022'] },
  ...
  { id: 'bentley', tier: 7, name: 'Dallen Bentley', pos: 'TE', team: 'DEN', college: 'Utah', pick: 256, s: ['REC  — 2025: 42-514-5TD (12.2)', 'MISC — Snow College → Utah; 12-game starter, top-10 national TE in receptions (2025)'] },
              ...
              <div key={pId} draggable={canEdit && boardView !== 'mock'} onDragStart={() => setDraggedId(pId)} onDragOver={(e) => { if (!canEdit || boardView === 'mock') return; e.preventDefault(); if (!draggedTierDiv && dragOverId !== p.id) setDragOverId(pId); }} onDrop={(e) => { if (draggedTierDiv) { setTierBreaks(prev => ({ ...prev, [draggedTierDiv]: rankIdx })); setDraggedTierDiv(null); setDragOverId(null); e.preventDefault(); } else { onDrop(e, pId); } }} onDragEnd={() => { setDraggedId(null); setDragOverId(null); }} style={{ background: isMockDrafted ? C.bg : flagColors.bg, border: `1px solid ${isMockDrafted ? C.border : flagColors.border}`, borderRadius: '4px', marginBottom: '5px', opacity: isMockDrafted ? 0.3 : (draggedId === p.id ? 0.4 : 1), position: 'relative' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <button disabled={!canEdit} onClick={() => moveByOne(idx, -1)} aria-label="Up" style={{ background: 'transparent', border: 'none', color: flagColors.color !== C.text ? flagColors.color : C.accent, cursor: canEdit ? 'pointer' : 'default', padding: '8px 4px', display: 'flex', opacity: canEdit ? 1 : 0.4 }}><ChevronUp size={16} /></button>
                    <button disabled={!canEdit} onClick={() => moveByOne(idx, 1)} aria-label="Down" style={{ background: 'transparent', border: 'none', color: flagColors.color !== C.text ? flagColors.color : C.accent, cursor: canEdit ? 'pointer' : 'default', padding: '8px 4px', display: 'flex', opacity: canEdit ? 1 : 0.4 }}><ChevronDown size={16} /></button>
                  </div>
                  {rankEditId === pId && canEdit ? (
                    <input
                      ...
                      style={{ width: '44px', fontSize: '16px', fontWeight: 700, color: C.accent, background: C.bg, border: `1px solid ${C.accent}`, borderRadius: '3px', textAlign: 'center', padding: '2px 0', touchAction: 'manipulation' }}
                    />
                  ) : (
                    <div
                      ...
                      style={{ fontSize: '17px', fontWeight: 700, color: flagColors.color !== C.text ? flagColors.color : C.accent, minWidth: '28px', textAlign: 'center', cursor: canEdit ? 'text' : 'default', touchAction: 'manipulation' }}
                    >
                      {rank}
                    </div>
                  )}
                  ...
                  <button disabled={!canEdit} onClick={() => toggleFlag(pId, 'target')} title={p.target ? 'Remove Target' : 'Mark as Target'} style={{ background: 'transparent', border: 'none', color: p.target ? C.target : C.textDim, cursor: canEdit ? 'pointer' : 'default', padding: '8px 4px', display: 'flex', opacity: canEdit ? 1 : 0.5 }}><Check size={15} /></button>
                  <button disabled={!canEdit} onClick={() => toggleFlag(pId, 'unlikely')} title={p.unlikely ? 'Remove Monitor' : 'Mark as Monitor'} style={{ background: 'transparent', border: 'none', color: p.unlikely ? C.unlikely : C.textDim, cursor: canEdit ? 'pointer' : 'default', padding: '8px 4px', display: 'flex', opacity: canEdit ? 1 : 0.5 }}>{p.unlikely ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                  <button disabled={!canEdit} onClick={() => toggleFlag(pId, 'noFit')} title={p.noFit ? 'Remove Avoid' : 'Mark as Avoid'} style={{ background: 'transparent', border: 'none', color: p.noFit ? C.noFit : C.textDim, cursor: canEdit ? 'pointer' : 'default', padding: '8px 4px', display: 'flex', opacity: canEdit ? 1 : 0.5 }}><X size={15} /></button>
                  {boardView === 'mock' && <button onClick={() => setMockDraftOrder(prev => isMockDrafted ? prev.filter(id => id !== pId) : [...prev, pId])} title={isMockDrafted ? 'Undo Draft' : 'Draft this player'} style={{ background: isMockDrafted ? `${C.textDim}22` : `${C.accent}22`, border: `1px solid ${isMockDrafted ? C.textDim : C.accent}88`, color: isMockDrafted ? C.textDim : C.accent, cursor: 'pointer', padding: '8px 10px', borderRadius: '3px', fontSize: '10px', letterSpacing: '1px', fontWeight: 700 }}>{isMockDrafted ? 'UNDO' : 'DRAFT'}</button>}
                  <button onClick={() => toggleExpand(pId)} aria-label={isExpanded ? 'Collapse' : 'Expand'} style={{ background: 'transparent', border: 'none', color: C.accent, cursor: 'pointer', padding: '8px 4px', display: 'flex' }}>{isExpanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</button>
                </div>
