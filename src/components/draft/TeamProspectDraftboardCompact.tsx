...
function calculatePositionNeeds(roster: Array<{ pos: string }>) {
  ...
  roster.forEach(p => { if (current[p.pos] !== undefined) current[p.pos]++; });

  return Object.keys(IDEAL_COUNTS).map((pos) => {
    ...
    return { pos, current: cur, ideal, need, priority };
...
export default function TeamProspectDraftboardCompact({
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        setSaveStatus(res.ok ? 'Synced' : 'Save failed');
      } catch {
  ...
  const toggleFlag = (id: string, flag: 'target' | 'unlikely' | 'noFit') => {
    setPlayers((prev) => prev.map((p) => p.id === id ? { ...p, [flag]: !p[flag] } : p));
  };
  ...
  const posRankMap: Record<string, number> = useMemo(() => {
  ...
  const bestAvailId = useMemo(() => {
    for (const p of players) {
      if (p.target && findAvailablePlayer(p) && !isPlayerDrafted(p)) return p.id;
    }
    return null;
  }, [players, findAvailablePlayer, isPlayerDrafted]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <div style={{ padding: '10px 12px', background: `${C.primary}14`, border: `1px solid ${C.border}`, borderRadius: '6px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {positionNeeds.map(n => (
            <div key={n.pos} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 8px', borderRadius: '4px', background: n.priority === 'high' ? `${C.primary}33` : n.priority === 'medium' ? `${C.unlikely}22` : `${C.border}`, border: `1px solid ${n.priority === 'high' ? C.primary : n.priority === 'medium' ? C.unlikely : C.border}` }}>
              ...
              {n.need > 0 && <span style={{ fontSize: '9px', fontWeight: 700, color: n.priority === 'high' ? C.primary : C.unlikely }}>+{n.need}</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          ...
          placeholder="Search..."
          style={{ flex: '1 1 120px', minWidth: '100px', padding: '6px 10px', background: C.bg, color: C.text, border: `1px solid ${C.border}`, borderRadius: '4px', fontSize: '12px' }}
        />
        <div style={{ display: 'flex', gap: '4px' }}>
          {['ALL', 'QB', 'RB', 'WR', 'TE', 'K', 'DEF'].map(pos => (
            <button key={pos} onClick={() => setPosFilter(pos)} style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, border: 'none', cursor: 'pointer', background: posFilter === pos ? (pos === 'ALL' ? C.accent : POS_COLORS[pos]) : C.border, color: posFilter === pos ? '#fff' : C.textMuted }}>
              {pos}
            </button>
          ))}
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: C.textMuted, cursor: 'pointer' }}>
          <input type="checkbox" checked={showDrafted} onChange={(e) => setShowDrafted(e.target.checked)} style={{ accentColor: C.accent }} />
          Show drafted
        </label>
        {saveStatus && <span style={{ fontSize: '10px', color: saveStatus === 'Synced' ? C.target : C.noFit, marginLeft: 'auto' }}>{saveStatus}</span>}
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: 'min(400px, 50dvh)', overflowY: 'auto' }}>
        {filteredPlayers.length === 0 ? (
          <div style={{ padding: '20px', textAlign: 'center', color: C.textDim, fontSize: '12px' }}>No players match your filters.</div>
        ) : filteredPlayers.map((p) => {
          const isDrafted = isPlayerDrafted(p);
          const availPlayer = findAvailablePlayer(p);
          const isAvail = !!availPlayer;
            ...
            <div key={p.id} style={{ background: isDrafted ? `${C.bg}88` : flagColors.bg, border: `1px solid ${isDrafted ? C.border : flagColors.border}`, borderRadius: '4px', opacity: isDrafted ? 0.55 : 1, position: 'relative' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '28px' }}>
                  <button disabled={isDrafted} onClick={() => moveByOne(rankIdx, -1)} style={{ background: 'transparent', border: 'none', color: C.accent, cursor: isDrafted ? 'default' : 'pointer', padding: '8px 6px', opacity: isDrafted ? 0.3 : 1 }}><ChevronUp size={14} /></button>
                  {rankEditId === p.id ? (
                    <input
                      type="number"
                      min={1}
                      ...
                      style={{ width: '36px', fontSize: '16px', fontWeight: 700, color: C.accent, background: C.bg, border: `1px solid ${C.accent}`, borderRadius: '3px', textAlign: 'center', padding: '1px 0' }}
                    />
                  ) : (
                    <span onClick={() => { if (!isDrafted) { setRankEditId(p.id); setRankEditVal(String(rank)); } }} title={isDrafted ? undefined : 'Click to jump to rank'} style={{ fontSize: '13px', fontWeight: 700, color: flagColors.color !== C.text ? flagColors.color : C.accent, cursor: isDrafted ? 'default' : 'text' }}>{rank}</span>
                  )}
                  <button disabled={isDrafted} onClick={() => moveByOne(rankIdx, 1)} style={{ background: 'transparent', border: 'none', color: C.accent, cursor: isDrafted ? 'default' : 'pointer', padding: '8px 6px', opacity: isDrafted ? 0.3 : 1 }}><ChevronDown size={14} /></button>
                </div>
                ...
                <button onClick={() => toggleFlag(p.id, 'target')} title="Target" style={{ background: 'transparent', border: 'none', color: p.target ? C.target : C.textDim, cursor: 'pointer', padding: '8px 4px' }}><Check size={14} /></button>
                <button onClick={() => toggleFlag(p.id, 'unlikely')} title="Monitor" style={{ background: 'transparent', border: 'none', color: p.unlikely ? C.unlikely : C.textDim, cursor: 'pointer', padding: '8px 4px' }}>{p.unlikely ? <EyeOff size={14} /> : <Eye size={14} />}</button>
                <button onClick={() => toggleFlag(p.id, 'noFit')} title="Avoid" style={{ background: 'transparent', border: 'none', color: p.noFit ? C.noFit : C.textDim, cursor: 'pointer', padding: '8px 4px' }}><X size={14} /></button>

                {isAvail && availPlayer && !isDrafted && canDraft && onDraft && (
                  <button onClick={() => onDraft(availPlayer)} title="Draft this player" style={{ padding: '8px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 900, letterSpacing: '0.5px', border: `2px solid ${teamColors?.secondary || C.primary}`, background: `linear-gradient(135deg, ${teamColors?.primary || C.primary}, ${teamColors?.secondary || C.accent})`, color: '#fff', cursor: 'pointer', textTransform: 'uppercase', boxShadow: `0 0 8px ${teamColors?.primary || C.primary}66` }}>
                    DRAFT
                  </button>
                )}
                {isAvail && availPlayer && !isDrafted && onAddToQueue && (
                  <button onClick={() => onAddToQueue(availPlayer)} title={isQueued ? 'In queue' : 'Add to queue'} style={{ padding: '8px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, border: `1px solid ${isQueued ? C.target : (teamColors?.secondary || C.accent)}`, background: isQueued ? `${C.target}22` : 'transparent', color: isQueued ? C.target : (teamColors?.primary || C.accent), cursor: 'pointer' }}>
                    {isQueued ? '✓' : 'Queue'}
                  </button>
                )}
                <button onClick={() => setExpandedId(prev => prev === p.id ? null : p.id)} style={{ background: 'transparent', border: 'none', color: C.accent, cursor: 'pointer', padding: '8px 4px' }}>
              ...
              {isExpanded && (
                <div style={{ padding: '8px 12px 12px', borderTop: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: '11px', color: C.textMuted, marginBottom: '8px' }}>{p.college} · NFL Pick #{p.pick}</div>
                  {p.s && p.s.length > 0 && (
                    <div style={{ marginBottom: '10px' }}>
                      <div style={{ fontSize: '9px', letterSpacing: '1.5px', color: C.accent, fontWeight: 700, marginBottom: '4px' }}>COLLEGE PRODUCTION</div>
                      <div style={{ fontSize: '11px', color: C.text, lineHeight: '1.5', padding: '6px 8px', background: `${C.bg}66`, borderRadius: '4px', border: `1px solid ${C.border}`, fontFamily: 'monospace' }}>
                        {p.s.map((line, i) => <div key={i} style={{ marginBottom: i < p.s.length - 1 ? '3px' : 0 }}>{line}</div>)}
                      </div>
                    </div>
                  )}
                  {p.userNote && (
                    <div>
                      <div style={{ fontSize: '9px', letterSpacing: '1.5px', color: C.accent, fontWeight: 700, marginBottom: '4px' }}>MY NOTES</div>
                      <div style={{ fontSize: '12px', color: C.text, fontStyle: 'italic', lineHeight: '1.4', padding: '6px 8px', background: `${C.bg}88`, borderRadius: '3px' }}>{p.userNote}</div>
                    </div>
                  )}
                  {!p.userNote && (!p.s || p.s.length === 0) && <div style={{ fontSize: '11px', color: C.textDim, fontStyle: 'italic' }}>No scouting data or notes available.</div>}
                </div>
              )}
            </div>
          );
      ...
      <a href="/draft?view=team-prospect-draftboard" target="_blank" rel="noopener noreferrer" style={{ display: 'block', textAlign: 'center', padding: '8px', fontSize: '11px', color: C.accent, textDecoration: 'none', border: `1px solid ${C.border}`, borderRadius: '4px', background: `${C.primary}11` }}>
