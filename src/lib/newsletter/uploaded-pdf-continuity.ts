...
function teamContextFromLine(line: string): string | null {
  ...
  for (const team of [...TEAM_NAMES].sort((a, b) => b.length - a.length)) {
    const index = lower.indexOf(team.toLowerCase());
    if (index < 0) continue;
    const looksLikeHeader =
      /^\d{1,2}\s*[/.)-]/.test(normalized) ||
      /POWER\s+RANKINGS?/i.test(normalized) ||
      /RECEIVES|TRADE GRADES|BOTTOM LINE|CENTRAL STORIES/i.test(normalized) ||
...
function isSourceOrSidebarLine(line: string): boolean {
  ...
  if (/^[A-Z][A-Za-z .&'-]{2,24}:\s+.{3,90}$/.test(line) && !/^(?:MASON REED|WESTY|TRENT)/i.test(line)) return true;
...
function isNoiseLine(line: string, title: string): boolean {
  ...
  if (line.length <= 90 && /^[A-Z0-9 "'&.,:/()\-]+$/.test(line) && !/[.!?]["']?$/.test(line)) {
    return true;
  }
  return false;
}
...
function speakerMarker(rawLine: string): { speaker: Speaker; remainder: string } | null {
  ...
  if (/^TRENT\s+["“”']?WESTY["“”']?\s+WESTON$/i.test(line) || /^TRENT WESTON$/i.test(line)) {
    return { speaker: 'westy', remainder: '' };
  }
  match = line.match(/^TRENT\s+["“”']?WESTY["“”']?\s+WESTON\s*[:\-–—]\s*(.+)$/i)
    ?? line.match(/^TRENT\s+["“”']?WESTY["“”']?\s+WESTON\s+(.+)$/i);
  if (match) {
    const remainder = normalizeLine(match[1]);
...
function likelyPlayerNames(rawText: string): string[] {
  ...
  for (const raw of candidates) {
    ...
    if (PERSON_NAME_STOP.has(value) || teamLower.has(value.toLowerCase())) continue;
...
function splitBySpeaker(rawText: string, title: string): {
  ...
  for (const rawLine of prepareRawText(rawText).split('\n')) {
    ...
    if (isHardBoundaryLine(line, title)) {
      if (active) flush();
      active = null;
      continue;
    }
