...
async function main() {
  ...
  if (!url) {
    console.error('[import] DATABASE_URL/POSTGRES_URL missing');
    process.exit(1);
  }
  const sql = neon(url);
  const items = parseBlocks(raw);
  if (items.length === 0) {
    console.log('[import] no items parsed');
    return;
  }
  // de-dup by id
  const map = new Map();
  ...
  console.log(`[import] inserted=${ok} skipped=${skip} total=${unique.length}`);
}
